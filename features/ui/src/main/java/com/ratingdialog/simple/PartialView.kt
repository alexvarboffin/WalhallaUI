...
class PartialView : RelativeLayout {
    ...
    fun setFilledDrawable(drawable: Drawable?) {
        val clipDrawable = ClipDrawable(drawable, Gravity.LEFT, ClipDrawable.HORIZONTAL)
        mFilledView!!.setImageDrawable(clipDrawable)
    }

    fun setEmptyDrawable(drawable: Drawable?) {
        val clipDrawable = ClipDrawable(drawable, Gravity.RIGHT, ClipDrawable.HORIZONTAL)
