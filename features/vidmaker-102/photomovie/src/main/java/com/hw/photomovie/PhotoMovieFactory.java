...
public class PhotoMovieFactory {
    ...
    //-16777216 в шестнадцатеричном представлении равно 0xFF000000
    //0xFF323232 в формате ARGB представляет собой темно-серый

    public static final int END_GAUSSIANBLUR_DURATION = 1500;
    public static final int MY_BLACK_COLOR = 0xFF000000;
    ...
    public static PhotoMovie generatePhotoMovie(PhotoSource photoSource, PhotoMovieType photoMovieType, int duration) {
        switch (photoMovieType) {
            case RANDOM:
                return randomPhotoMovie(photoSource, duration);
            case THAW:
                return generateThawPhotoMovie(photoSource, duration);
            case SCALE:
                return generateScalePhotoMovie(photoSource, duration);
            case SCALE_TRANS:
                return generateScaleTransPhotoMovie(photoSource, duration);
            case WINDOW:
                return generateWindowPhotoMovie(photoSource, duration);
            case HORIZONTAL_TRANS:
                return generateHorizontalTransPhotoMovie(photoSource, duration);
            case VERTICAL_TRANS:
                return generateVerticalTransPhotoMovie(photoSource, duration);
            case GRADIENT:
                return genGradientPhotoMovie(photoSource, duration);
            case TEST:
    ...
    private static PhotoMovie generateHorizontalTransPhotoMovie(PhotoSource photoSource, int i) {
        ...
        for (int i2 = 0; i2 < photoSource.size(); i2++) {
            arrayList.add(new FitCenterSegment(i).setBackgroundColor(MY_BLACK_COLOR));
    ...
    private static PhotoMovie generateWindowPhotoMovie(PhotoSource photoSource, int i) {
        ...
        arrayList.add(new EndGaussianBlurSegment(END_GAUSSIANBLUR_DURATION));
        return new PhotoMovie(photoSource, arrayList);
    }
