...
public class PhotoSource implements PhotoData.OnDataLoadListener {
    ...
    private final Map<PhotoData, Integer> mErrorMap = new ConcurrentHashMap();
    private OnSourcePrepareListener mOnSourcePrepareListener;
    private final List<PhotoData> mPhotoDataList = new Vector();
    private final SparseArray<Float> mProgressArray = new SparseArray<>();
    ...
    private synchronized void notifyPrepareProgress() {
        ...
        while (i < size() && i < this.mRequiredDiskPrepareNum) {
            f += (this.mProgressArray.get(get(i).hashCode(), Float.valueOf(0.0f)).floatValue()) / ((float) this.mRequiredDiskPrepareNum);
            i++;
        }
        ...
        DLog.i(TAG, "onDownloadProgressUpdate:" + f);
    }
    ...
    public void onError(PhotoData photoData, ErrorReason errorReason) {
        ...
        if (valueOf.intValue() >= 2) {
            DLog.e(TAG, photoData + " prepare error:" + valueOf + " 放弃加载。");
            this.mPhotoDataList.remove(photoData);
            notifyPrepareProgress();
            return;
        }
        photoData.prepareData(photoData.getTargetState(), this);
        DLog.e(TAG, photoData + " prepare error:" + valueOf);
    }
}
