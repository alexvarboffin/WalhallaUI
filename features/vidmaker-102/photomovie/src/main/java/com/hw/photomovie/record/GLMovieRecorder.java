...
public class GLMovieRecorder {
    ...
    public void setMusic(Uri audioPath){
    ...
    public void startRecordImpl(final OnRecordListener listener) throws IOException {
        ...
        if(mAudioPath!=null&&!TextUtils.isEmpty(mAudioPath.getPath())){
        ...
        try {
            while (true) {
                ...
                DLog.i(TAG, "com.hw.photomovie.record frame " + frameCount);
                DLog.i(TAG, "com.hw.photomovie.record 耗时 " + (e - s) + "ms" + " 绘制耗时:" + (e1 - s1) + "ms");
                frameCount++;
