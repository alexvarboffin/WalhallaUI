...
class GameSoundEffects(private val context: Context) {
    ...
    init {
        ...
        SoundGenerator.generateSpinningSound(context)
        SoundGenerator.generateStopSound(context)
        SoundGenerator.generateWinSound(context)
        ...
        // Инициализируем звук вращения
        spinningSound = MediaPlayer().apply {
            setDataSource(File(context.filesDir, "slot_spinning.wav").absolutePath)
        ...
        winSound = MediaPlayer().apply {
            setDataSource(File(context.filesDir, "slot_win.wav").absolutePath)
            setAudioAttributes(audioAttributes)
            isLooping = false
            setVolume(1.0f, 1.0f)
            prepare()
        ...
        // Инициализируем звук остановки барабана
        stopSound = MediaPlayer().apply {
            setDataSource(File(context.filesDir, "slot_stop.wav").absolutePath)
            setAudioAttributes(audioAttributes)
            isLooping = false
            setVolume(1.0f, 1.0f)
            prepare()
